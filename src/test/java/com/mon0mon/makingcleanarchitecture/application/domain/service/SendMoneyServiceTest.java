...
class SendMoneyServiceTest {
    ...
    void givenWithdrawalFails_thenOnlySourceAccountIsLockedAndReleased() {
        ...
        then(accountLock).should()
                         .lockAccount(eq(sourceAccountId));
        then(sourceAccount).should()
                           .withdraw(eq(money), eq(targetAccountId));
        then(accountLock).should()
                         .releaseAccount(eq(sourceAccountId));
        then(accountLock).should(times(0))
                         .lockAccount(eq(targetAccountId));
    }
    ...
    void transactionSucceeds() {
        ...
        then(targetAccount).should()
                           .deposit(eq(money), eq(sourceAccountId));
        then(accountLock).should()
                         .releaseAccount(eq(targetAccountId));
    ...
    private Account givenAnAccountWithId(AccountId id) {
        ...
        given(loadAccountPort.loadAccount(eq(account.getId()
